...
import adminService from "./adminSevice";
...
const adminSlice = createSlice({
    ...
    extraReducers: builder =>{
        ...
        .addCase(updateEvent.fulfilled,(state,action)=>{
            state.adminLoading = false
            state.adminSuccess = true
            state.allEvents  = state.allEvents.map(event => event.id === action.payload._id ? action.payload:event)
...
export const updateListing = createAsyncThunk("UPDATE/LISTINGS/ADMIN",async(updatedProduct,thunkAPI)=>{
    let token = thunkAPI.getState().auth.user.token 
